"""
Transformer04_ModelFromWayLandZhang - 在CPU运行的小数据集的微调测试
备注：
    已跑通
    需要梯子
    超参处max_iters降低到500之内，太高的话，CPU跑不动非常耗时，其他超参类似，数值不要大
    source: https://github.com/waylandzhang/Transformer-from-scratch/blob/master/model.py
                    By: Fisherlen Yu
                    Date: 2024/4/24
"""
...
learning_rate = 1e-3  # 0.001 学习率越小，训练越稳定，但是训练越慢
dropout = 0.1  # Dropout rate 随机丢弃率（丢弃10%的token，让模型更稳定）
max_iters = 200  # Total of training iterations <- Change this to smaller number for testing 迭代次数
...
# ----------------------------------------------------Load training data, tokenize, split train and validation-----------
...
if not os.path.exists('sales-text_book.txt'):
    ...
    with open('sales-text_book.txt', 'wb') as f:
...
# ----------------------------------------------------Initialize the model, Train the model------------------------------
